...
//
// Only runs when the model's canonical components include all four required types: Business
// Capability, Business Process Flow, Task, and Application. If any are missing, this is a no-op
// — the component load itself is unaffected.

const CanonicalComponent = require('../models/CanonicalComponent');
const Diagram = require('../models/Diagram');
const { buildBpmnXmlForFlow } = require('./bpmnXmlBuilder');

const REQUIRED_TYPES = ['Business Capability', 'Business Process Flow', 'Task', 'Application'];
...
function typeMatches(componentType, name) {
  return String(componentType || '').trim().toLowerCase() === name.toLowerCase();
}

async function getFlowLineage(flowDoc) {
  const lineage = { businessCapability: null, subdomain: null, domain: null };
  ...
  for (let hop = 0; hop < 6 && currentRefs.length; hop += 1) {
    ...
    if (typeMatches(parent.componentType, 'Business Capability')) lineage.businessCapability = parentName;
...
async function generateDiagramForFlow(neighborhoodName, flow) {
  ...
  const { breadcrumb, businessCapability, domain, subdomain } = await getFlowLineage(flow);
