...
function buildBpmnXmlForFlow({ flowName, breadcrumb, tasks }) {
  ...
  taskNodes.forEach((t) => {
    ...
    const annotationId = toId('TextAnnotation', t.name, usedIds);
    const associationId = toId('Association', t.name, usedIds);
    annotations.push({ id: annotationId, associationId, text: t.applications.join(', '), targetId: t.id });
  });

  const titleAnnotationId = toId('TextAnnotation', 'DiagramTitle', usedIds);
  const titleText = breadcrumb || `Business Flow: ${flowName}`;

  const annotationsXml = [
    `    <bpmn:textAnnotation id="${titleAnnotationId}">\n      <bpmn:text>${escapeXml(titleText)}</bpmn:text>\n    </bpmn:textAnnotation>`,
    ...annotations.map((a) => (
  ...
  // ── layout / DI ─────────────────────────────────────────────────────
  const diagramWidth = START_X + Math.max(taskNodes.length, 1) * STEP_X + 140;
  ...
  // Title annotation sits above the whole diagram, top-left.
  const titlePos = { x: LANE_X, y: Math.max(0, TOP_Y - LANE_LABEL_MARGIN - ANNOTATION_HEIGHT), width: 360, height: ANNOTATION_HEIGHT };

  const annotationShapesAndEdges = [
    `      <bpmndi:BPMNShape id="${titleAnnotationId}_di" bpmnElement="${titleAnnotationId}">\n` +
    `        <dc:Bounds x="${titlePos.x}" y="${titlePos.y}" width="${titlePos.width}" height="${titlePos.height}"/>\n` +
    `      </bpmndi:BPMNShape>`,
    ...annotations.map((a) => {
      const taskPos = positions.get(a.targetId);
      const annoPos = {
        x: taskPos.x - (ANNOTATION_WIDTH - TASK_WIDTH) / 2,
        y: taskPos.y - ANNOTATION_HEIGHT - ANNOTATION_GAP,
        width: ANNOTATION_WIDTH,
        height: ANNOTATION_HEIGHT,
      };
      positions.set(a.id, annoPos);
      const from = { x: annoPos.x + annoPos.width / 2, y: annoPos.y + annoPos.height };
      const to = centerOf(taskPos);
      return (
